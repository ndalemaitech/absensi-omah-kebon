...
(async function () {
  ...
  await test_run('Login pertama Owner (Mas Abim): buat PIN, dashboard tampil semua tab', async function () {
    var win = buatDevice();
  ...
  await test_run('Login HR (Bu Lis): tab lembur, rekap, akun tersembunyi sesuai izin default', async function () {
    var win = buatDevice();
    ...
    assert.strictEqual(win.document.querySelector('.admin-tab-item[data-tab="pin"]').classList.contains('admin-tab-tersembunyi'), false);
  });

  await test_run('Login Rekap (Mbak Tika): tab lembur & rekap tampil, akun tersembunyi', async function () {
    var win = buatDevice();
  ...
  await test_run('PIN salah saat login ditolak dengan pesan error', async function () {
    ...
    klik(win, 'admin-btn-lanjut-akun');
    backend.doPost({ action: 'adminLogin', id_admin: 'ADM001', pin: '1111' }); // set PIN duluan lewat backend langsung
  ...
  await test_run('Kelola Akun Admin (Owner): toggle izin checkbox tersimpan ke backend', async function () {
    var win = buatDevice();
    await loginAdmin(win, 'ADM001', '1111');
    pindahTabUji(win, 'akun');
    await tick(8);
    var cekApprove = win.document.querySelector('.admin-cek-izin[data-id="ADM003"][data-izin="izin_approve_pengajuan"]');
    assert.strictEqual(cekApprove.checked, false);
    cekApprove.checked = true;
    cekApprove.dispatchEvent(new win.Event('change', { bubbles: true }));
    await tick(8);
  ...
  await test_run('Kelola Akun Admin: reset PIN admin lain memaksa mereka buat PIN baru', async function () {
    backend.doPost({ action: 'adminLogin', id_admin: 'ADM002', pin: '2222' });
    var win = buatDevice();
    await loginAdmin(win, 'ADM001', '1111');
    pindahTabUji(win, 'akun');
    await tick(8);
    var btnReset = win.document.querySelector('.admin-btn-reset-pin[data-id="ADM002"]');
    btnReset.dispatchEvent(new win.Event('click', { bubbles: true }));
    await tick(8);
    var res = backend.doPost({ action: 'adminLogin', id_admin: 'ADM002', pin: '2222' });
